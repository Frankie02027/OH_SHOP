    ...
    if config.sandbox is None:
        ...
        if os.getenv('RUNTIME') == 'remote':
            config.sandbox = RemoteSandboxServiceInjector(
                ...
                I validated the placeholder status; now I’m testing real Python imports for each wrapper in the active env. That will show whether those five are being pruned due actual dependency breakage (not placeholder files).
