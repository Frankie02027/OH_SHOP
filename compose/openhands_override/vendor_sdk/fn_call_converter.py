...
# Note: The >? makes the closing bracket after the function name optional,
# because some local models emit <function=name\n<parameter=... without the >.
...
def convert_tool_call_to_string(tool_call: dict) -> str:
    ...
    if "function" not in tool_call:
        raise FunctionCallConversionError(
            "Tool call must contain 'function' key.")
    ...
    try:
        ...
        raise FunctionCallConversionError(
            ...
            f"Arguments: {tool_call['function']['arguments']}") from e
...
def _summarize_schema_type(schema: object | None) -> str:
    ...
    for key in SCHEMA_UNION_KEYS:
        if key in schema:
            return " or ".join(
                _summarize_schema_type(option) for option in schema[key])
    ...
    if schema_type == "array":
        ...
        if isinstance(items, list):
            item_types = ", ".join(
                _summarize_schema_type(item) for item in items)
...
def _format_union_details(schema: dict[str, object],
                          indent: int) -> list[str] | None:
    for key in SCHEMA_UNION_KEYS:
        ...
        for option in options:
            ...
            lines.extend(
                _format_schema_detail(option, _nested_indent(indent, 2)))
        return lines
    return None
...
def _format_array_details(schema: dict[str, object], indent: int) -> list[str]:
    ...
    if isinstance(items, list):
        for index, item_schema in enumerate(items):
            ...
            lines.extend(
                _format_schema_detail(item_schema, _nested_indent(indent, 2)))
...
def _format_additional_properties(additional_props: object | None,
                                  indent: int) -> list[str]:
    if isinstance(additional_props, dict):
        line = (f"{_indent(indent)}Additional properties allowed: "
                f"{_summarize_schema_type(additional_props)}")
        lines = [line]
        lines.extend(
            _format_schema_detail(additional_props, _nested_indent(indent)))
        return lines
    ...
    for message in messages:
            ...
            if "tool_calls" in message and message["tool_calls"] is not None:
                if len(message["tool_calls"]) != 1:
                    raise FunctionCallConversionError(
                        ...
                        f"Content: {content}")
                try:
                    tool_content = convert_tool_call_to_string(
        ...
        # 4. TOOL MESSAGES (tool outputs)
            ...
            if isinstance(content, str):
                ...
                if content and (first_text_content := next(
                    (c for c in content if c["type"] == "text"), None)):
                    first_text_content[
                        "text"] = prefix + first_text_content["text"]
                else:
                    content = [
                        cast(TextPart, {
                            "type": "text",
...
def _extract_and_validate_params(
    ...
    required_params = set()
    if "parameters" in matching_tool and "required" in matching_tool[
            "parameters"]:
        required_params = set(matching_tool["parameters"].get("required", []))

    allowed_params = set()
    if "parameters" in matching_tool and "properties" in matching_tool[
            "parameters"]:
    ...
    for param_match in param_matches:
        ...
        # Enum check
        if ("parameters" in matching_tool and "enum"
                in matching_tool["parameters"]["properties"][param_name]):
            if (param_value not in matching_tool["parameters"]["properties"]
                [param_name]["enum"]):
                raise FunctionCallValidationError(
...
def _normalize_model_tool_markup(content: str) -> str:
...
def _fix_stopword(content: str) -> str:
    ...
    for marker in ('<|im_start|>', '<|im_end|>'):
        marker_idx = content.find(marker,
                                  first_function_idx + len('<function='))
        if marker_idx != -1:
            end_candidates.append(marker_idx)

    next_function_idx = content.find('<function=',
                                     first_function_idx + len('<function='))
...
def convert_non_fncall_messages_to_fncall_messages(
    ...
    for message in messages:
        ...
        # Skip user messages (no conversion needed)
            ...
            if not first_user_message_encountered:
                ...
                if isinstance(content, str):
                    ...
                    if content.startswith(
                            IN_CONTEXT_LEARNING_EXAMPLE_PREFIX(tools)):
            ...
            if isinstance(content, str):
                tool_result_match = re.search(TOOL_RESULT_REGEX_PATTERN,
                                              content, re.DOTALL)
            elif isinstance(content, list):
                tool_result_match = next(
                    (_match for item in content
                     if item.get("type") == "text" and (_match := re.search(
            ...
            if tool_result_match:
                ...
                converted_messages.append({
                    ...
                    "content": [{
                        "type": "text",
                        "text": tool_result
                    }] if isinstance(content, list) else tool_result,
                    "tool_call_id":
                    f"toolu_{tool_call_counter - 1:02d}",
        ...
        # Handle assistant messages
            ...
            if isinstance(content, str):
                ...
                if content and content[-1]["type"] == "text":
                    ...
                    fn_match = re.search(FN_REGEX_PATTERN, content[-1]["text"],
                                         re.DOTALL)
                else:
                    fn_match = None
                fn_match_exists = any(
            ...
            if fn_match:
                ...
                # Parse parameters
                param_matches = re.finditer(FN_PARAM_REGEX_PATTERN, fn_body,
                                            re.DOTALL)
                params = _extract_and_validate_params(matching_tool,
                                                      param_matches, fn_name)
    ...
    for message in messages:
        ...
        if role == "assistant":
            ...
            if message["tool_call_id"] in pending_tool_calls:
                ...
                _tool_call_message = pending_tool_calls.pop(
