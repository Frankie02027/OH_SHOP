...
| `ops/manual/chat_guard.py` | reconciles stale chat rows / orphan containers | some runtime cleanup remains manual/provisional | optional manual or compose sidecar usage | Keep as manual repair tool, not architecture |
| `ops/manual/lmstudio-docker-dnat.sh` | installs privileged host routing workaround | useful only on hosts where `host.docker.internal` is not enough | manual host ops only | Keep manual-only; never treat as baseline |
